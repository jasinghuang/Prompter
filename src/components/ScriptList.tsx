        ...
        <div className="mx-auto flex max-w-5xl items-center gap-3">
          ...
          <div className="relative ml-auto">
            ...
            <input
              ...
              className="w-28 rounded-full border border-neutral-800 bg-neutral-900 py-2 pl-8 pr-2.5 text-sm text-white placeholder-neutral-600 transition-colors focus:w-48 focus:border-[#D4A432] focus:outline-none sm:w-48"
            />
          </div>
          <button
            onClick={onCreate}
            ...
            <div className="flex items-center gap-1">
              ...
              {SORT_OPTIONS.map((opt) => (
                <button
                  ...
                  className={sort === opt.value ? 'text-[#D4A432]' : 'text-neutral-500 hover:text-neutral-300 transition-colors'}
                >
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {filtered.map((s) => (
              <div
                key={s.id}
                onClick={() => onOpen(s.id)}
                className="group relative cursor-pointer rounded-2xl border border-[#26262B] bg-[#131316] p-4 transition-all hover:border-[rgba(212,164,50,0.2)] hover:bg-[#1A1A1F]"
              >
                {/* Title Row */}
                <div className="mb-2 flex items-start justify-between">
                  ...
                  <div className="flex shrink-0 gap-0.5 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      ...
                      className="rounded-lg p-1.5 text-neutral-400 hover:bg-red-500/10 hover:text-red-400 transition-colors"
                      aria-label="删除稿件"
                    >
                      <Trash2 size={14} />
                    </button>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-3xl border border-neutral-800 bg-neutral-900 p-8">
            ...
            <div className="flex gap-3">
              <button onClick={() => setConfirmClear(false)} className="flex-1 rounded-xl bg-neutral-800 py-3 text-sm font-bold text-white transition-colors hover:bg-neutral-700">
                取消
              </button>
              <button
                onClick={() => { onDeleteAll?.(); setConfirmClear(false); }}
                className="flex-1 rounded-xl bg-red-500 py-3 text-sm font-bold text-white transition-all active:scale-95"
              >确认清空</button>
            </div>
          </div>
        </div>
      ...
      {confirmId && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-3xl border border-neutral-800 bg-neutral-900 p-8">
            ...
            <div className="flex gap-3">
              <button onClick={() => setConfirmId(null)} className="flex-1 rounded-xl bg-neutral-800 py-3 text-sm font-bold text-white transition-colors hover:bg-neutral-700">
                取消
              </button>
              <button
